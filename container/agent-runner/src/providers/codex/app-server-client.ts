 ...
 * Ported from v1, with onEvent callback for real-time activity events.
 */
...
export interface AppServerInputItem {
...
interface ActiveTurn {
  ...
  onEvent?: (event: AppServerTurnEvent) => void;
...
export class CodexAppServerClient {
  ...
  /** Register a callback for unexpected process exits. */
  setOnProcessExit(cb: (error: Error) => void): void {
    this.onProcessExit = cb;
  }
  ...
  async start(): Promise<void> {
    ...
    this.proc.stderr.on('data', (chunk: string) => {
      for (const line of chunk.split('\n')) {
        ...
        if (/panicked at/.test(trimmed)) {
          this.log('Detected Rust panic in app-server — killing process');
  ...
  /** Force-kill the app-server process (used for crash recovery). */
  kill(): void {
    if (!this.proc) return;
    try {
      this.proc.kill('SIGKILL');
  ...
  async startOrResumeThread(
    ...
    options: { cwd: string; model?: string; developerInstructions?: string },
  ): Promise<string> {
  ...
  async startTurn(
    ...
    try {
      ...
      if (activeTurn !== null) {
        const event: AppServerTurnEvent = {
          method: 'turn/started',
          params: {
            turn: {
              id: turnId,
        ...
        activeTurn.state = reduceAppServerTurnState(activeTurn.state, event);
  ...
  private handleResponse(message: JsonRpcResponse): void {
    ...
    if (message.error) {
      pending.reject(
        new Error(message.error.message || `${pending.method} failed with JSON-RPC error ${message.error.code ?? 'unknown'}`),
      );
      return;
    }
  ...
  private handleNotification(message: JsonRpcNotification): void {
    ...
    // Forward every notification as an event for real-time activity tracking
    this.activeTurn.onEvent?.(message as AppServerTurnEvent);
    ...
    this.activeTurn.state = reduceAppServerTurnState(this.activeTurn.state, message as AppServerTurnEvent);
