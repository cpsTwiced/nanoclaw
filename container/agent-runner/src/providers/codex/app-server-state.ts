...
export function reduceAppServerTurnState(
  ...
  if (event.method === 'item/completed') {
    ...
    if (item.type === 'agentMessage') {
      const text = typeof item.text === 'string' && item.text.trim().length > 0 ? item.text : null;
  ...
  if (event.method === 'error') {
    ...
    return {
      ...state,
      errorMessage: typeof httpStatusCode === 'number' ? `${message} (HTTP ${httpStatusCode})` : message,
    };
  }
  ...
  if (event.method === 'turn/completed') {
    ...
    const status =
      turn?.status === 'completed' || turn?.status === 'failed' || turn?.status === 'interrupted'
        ? turn.status
        : 'completed';
    const turnError = typeof turn?.error === 'string' ? turn.error : (turn?.error?.message || null);
...
export function getAppServerTurnResult(state: AppServerTurnState): string | null {
